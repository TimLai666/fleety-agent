...
- Replacing ratatui, changing the config/provider formats, or redesigning the agent protocol beyond optional read-result metadata required for machine output. The connection profile format may extend its already-opaque generation so current binaries can detect security state removed by an older writer.
...
- Re-running explicit `init` against the same endpoint of an existing profile is still an operational use of that durable owner, not a fresh enrollment. After any legacy-generation migration, one authoritative snapshot alone supplies its endpoint, token, pin, secure latch, transport target, `Hello`, and commit preconditions; no credential or security decision survives from the earlier preflight snapshot. The same-URL owner is retained even when authentication rejection has cleared its token, so a latched tokenless profile fails before transport rather than becoming ownerless. Existing paired profiles attempt the secure candidate handshake before placing `Hello` and the saved token inside the encrypted channel. Moving an existing profile to a different URL also requires explicit pairing whenever any token, pin, secure latch, learned endpoint, or separately configured endpoint shows that the profile has protected connection state. Only a new enrollment, a genuinely unprotected profile, or explicit pairing-code recovery remains ownerless and token-free on the existing cleartext path.
...
- The opaque profile generation also carries a versioned digest of whether `endpoints`, `configured_url`, and `secure` were present when a current binary last wrote the profile. Older binaries preserve the opaque generation even though they ignore those newer fields. A current binary compares the digest before resolving or mutating the profile and fails closed on mismatch. This detects a stale writer without adding a second state file or pretending a top-level version field can survive an older serializer. A legacy unversioned generation is upgraded only through the existing durable-profile migration path. This cannot make the stale executable itself secure while it is running, so the error requires updating every installed Fleety binary and explicitly re-pairing the affected profile.
- Selecting a legacy profile with `connection use` makes it operational, so the selected generation is upgraded inside the same connection-store mutation lease that changes `current`; no later resolver window may bind state after an older writer has already removed it. Recovery text also follows the state it can safely repair: a generation that records a now-missing configured address points directly to explicit-URL `init --pairing-code`, never to bare `pair`, which deliberately refuses to guess a learned primary.
- A Settings profile switch sets the selected profile current and resolves it through `Target::Current` inside the same connection-store lease. The resulting target therefore requires that exact current owner to survive until transport; if another process switches again before the reconnect, Settings reports a conflict and loads no snapshots from the stale Server.
- Every client surface — one-shot commands, Chat and its reconnect, ACP turns and cancels, Settings, the Provider editor, `doctor`, and the Daemon — opens sessions through one helper, so the channel policy, the per-candidate deadline, and the rule that nothing commits before the Server is proven cannot drift apart. `doctor` passes through it read-only, then bounds each post-`Welcome` Provider／Daemon snapshot request separately so a proven but silent Server becomes a diagnostic result instead of hanging the command. Because config snapshot replies have no request identifier, any snapshot timeout invalidates the diagnostic stream: Doctor closes it and marks later owner checks blocked instead of allowing a delayed earlier reply to satisfy a later request.
- The Daemon creates one mandatory deadline before each candidate transport attempt and carries that exact deadline through transport open, `Hello` send, and authenticated `Welcome`, for both ordinary automatic connection and owner-requested reconnect. No transport mode may bypass it; in particular, an SSE downstream that opens while its POST upstream stalls must release the candidate loop.
- A durable profile connects to its last successful endpoint first, then its learned endpoints in stable order. Before every candidate transport, including an advance after an earlier endpoint stalls or fails, the client revalidates the exact frozen profile owner against `connections.toml`; this includes Doctor's read-only diagnostic owner without restoring mutation authority. A concurrent pairing, profile mutation, or incompatible writer aborts the remaining sweep before another endpoint receives the stale token／pin. A candidate becomes primary only after `Welcome` returns the profile's pinned Server identity. Failed, malformed, duplicate, loopback-only, or identity-mismatched candidates are not promoted. Raw／environment targets remain single-endpoint and never learn into a profile. Doctor relies on the shared aggregate sweep bound instead of cancelling it early; Settings keeps its shorter whole-operation deadline by dividing that budget across the same maximum number of candidates.
...
- Durable profile schema additions, all serde-default so an older file loads unchanged: `endpoints` (Server-advertised alternates), `configured_url` (the address the user chose, once roaming has moved the primary), and `secure` (this Server has proven it can open the encrypted channel). The opaque `generation` uses a versioned envelope containing its lifecycle nonce and the presence digest for those three fields. Legacy plain generations remain readable until the selected durable profile is migrated. A versioned generation whose digest no longer matches its fields is an incompatible-writer error, not a legacy profile. The local reconnect runtime-control artifacts use the versioned compatibility contract below.
...
- Diagnostic reply timeout: `doctor` retains the authenticated Server result, marks the silent Provider or Daemon snapshot check as failed／warned with remediation, and returns without waiting indefinitely.
...
- Reconnect journal append failure: inability to read the original journal length aborts before writing. A write or sync failure truncates only through a separate writable handle back to the measured length, so Windows append semantics and rollback failure cannot erase previously durable events.
- Incompatible connection writer: when a versioned profile generation says roaming or secure-channel state existed but the corresponding fields are absent or changed incompatibly, every current CLI, TUI, ACP, doctor, and Daemon path fails before sending a credential or accepting control. The error identifies the profile and tells the user to update every Fleety binary and re-pair. A legacy unversioned profile is migrated only when the existing owner rules authorize a durable write.
...
- Reconnect crash tests stop after an owner-drift receipt becomes durable but before the journal is reaped, then restart and prove the existing receipt is accepted, the journal is removed, and `ControlGuard::claim` succeeds. Append tests inject metadata, write, sync, rollback-open, and rollback-truncate failures over a journal with prior events and verify those events remain byte-identical on Unix and Windows-compatible handles.
- Connection compatibility tests serialize a profile with learned endpoints, a configured address, and a secure latch, simulate an older serializer dropping each new field while preserving the opaque generation, and prove every current resolution and mutation path fails before transport or credential use. Legacy plain generations remain usable until the authorized durable migration rewrites them into the versioned envelope.
