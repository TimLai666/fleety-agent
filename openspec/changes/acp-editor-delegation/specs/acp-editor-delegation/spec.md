...
In an editor-served conversation the system prompt SHALL instruct the agent to
prefer the editor-backed tools for the user's work, and SHALL explain that editor
edits live in the buffer (may be unsaved and require the user's approval) while
disk reads do not reflect unsaved buffer edits until saved. Editor tool results
SHALL carry their surface and saved state.

#### Scenario: prompted to prefer editor tools

- **WHEN** a conversation is served by an editor
- **THEN** the agent's system prompt instructs it to prefer the editor-backed tools and to ask the user to save when disk must reflect editor edits
