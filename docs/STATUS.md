...
- **skills + MCP runtime** — `list_skills`/`use_skill` load `SKILL.md` packs (builtin/installed, installed overrides, symlink-skipped); `mcp_list`/`mcp_add`/`mcp_remove`/`mcp_call` configure and call external MCP servers over stdio JSON-RPC. Server tools: 28.
...
Still open — the **heavier, external/UI-bound** items needing their own integration environment: device-scoping enforcement on handles (#5, needs multi-device handles), client_session tool bridge (#11), SSE streaming (incremental display), connectors (SSH/HTTP transports + daemon), fleetyd daemon (heartbeat/autostart/on-device tools), ratatui TUI, browser CDP, computer-use, knowledge wiki, updater, schedule mandate enforcement.
