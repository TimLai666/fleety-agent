...
- **browser automation (CDP)** — `browser_navigate`/`browser_eval`/`browser_screenshot` drive a Chrome started with `--remote-debugging-port` over the DevTools Protocol (page discovery via `GET {base}/json`, commands over a WebSocket; zero new deps). `FLEETY_CHROME_URL` or a `chrome` arg targets the instance. Screenshot is the low-impact observation (risk=read). **computer-use** is delivered via the MCP runtime (register `computer-use-mcp` with `mcp_add`/`mcp_call`). Server tools: 38.
