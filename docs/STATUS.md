...
Still open — the **heavier, external/UI-bound** items needing their own integration environment: device-scoping enforcement on handles (#5, needs multi-device handles), client_session tool bridge (#11), SSE streaming (incremental display), connectors (SSH/HTTP transports + daemon), fleetyd daemon (heartbeat/autostart/on-device tools), ratatui TUI, browser CDP, computer-use, updater, schedule mandate enforcement.
