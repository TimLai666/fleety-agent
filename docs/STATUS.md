...
- **ssh_exec connector** — run commands on remote hosts over the system `ssh` client (non-interactive BatchMode; user/port/identity; host guarded against ssh option-injection). Zero new deps; lets the agent operate hosts without fleetyd. Server tools: 35.
...
Still open — the **heavier, external/UI-bound** items needing their own integration environment: device-scoping enforcement on handles (#5, needs multi-device handles), client_session tool bridge (#11), SSE incremental token display (consumption done; needs protocol deltas + TUI), ratatui TUI + SSE incremental display (needs an interactive terminal to render/verify), browser CDP (needs a real Chrome instance), computer-use (needs a real desktop + input/capture), device-scoping handle enforcement (downstream of browser/SSH *session handles* — `device_exec`/`ssh_exec` are stateless and already validate their target; persistent-handle binding awaits those handle types). **Every no-dependency, in-repo, verifiable item is implemented; the remaining four are all external-GUI/hardware or interactive-terminal bound and must be built+verified in those environments.**
