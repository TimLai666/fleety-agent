...
Still open — the **heavier, external/UI-bound** items needing their own integration environment: device-scoping enforcement on handles (#5, needs multi-device handles), client_session tool bridge (#11), SSE incremental token display (consumption done; needs protocol deltas + TUI), SSH connector transport (needs an external ssh crate + a live server to verify), ratatui TUI + SSE incremental display (terminal UI), browser CDP (real Chrome), computer-use (external desktop), device-scoping handle enforcement (downstream of browser/SSH handles — `device_exec` already validates the target device is connected; full handle-binding awaits those handle types). **The remaining items are all external-system or terminal-UI bound; the in-repo architecture is complete.**
