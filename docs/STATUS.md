...
- **durable turn execution** — each turn's loop events are journaled to `conversations/{id}.journal.jsonl` the instant they happen (via an `EventLog` sink), so a turn interrupted by a crash/redeploy isn't lost: it is reconstructed (`reconstruct_messages`) and continued to completion — interactive turns on the next message/`Resume`, scheduled turns on the next tick (mandate restored from the schedule). The in-flight tool at the crash is flagged **interrupted, never re-run** (safe under full access). Unit-tested (sink, reconstruct, journal lifecycle) plus an end-to-end scheduler recovery test.
