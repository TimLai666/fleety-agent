...
Still open — the **heavier, external/UI-bound** items needing their own integration environment: device-scoping enforcement on handles (#5, needs multi-device handles), client_session tool bridge (#11), SSE incremental token display (consumption done; needs protocol deltas + TUI), connectors (HTTP capability done via http_request; SSH transport needs an external crate + live server), fleetyd daemon (autostart/on-device tools; heartbeat is already covered by WebSocket control-frame ping/pong), ratatui TUI, browser CDP, computer-use, updater.
