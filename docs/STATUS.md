...
Still open — the **heavier, external/UI-bound** items needing their own integration environment: device-scoping enforcement on handles (#5, needs multi-device handles), client_session tool bridge (#11), SSE incremental token display (consumption done; needs protocol deltas + TUI), ratatui multi-pane TUI + SSE incremental token display (the CLI `fleety ask`/`resume`/`init` is the working v0 terminal client; the rich multi-pane TUI is fundamentally interactive and needs a TTY to render/verify — its rendering can't be checked headlessly), and device-scoping handle enforcement (a refinement that is downstream of *stateful* session handles — the current `device_exec`/`ssh_exec`/`browser_*` tools are stateless and already validate their target, so there is no device-bound handle to enforce yet). **Everything implementable and verifiable in a headless context is done; what remains is interactive-terminal UI polish and a refinement that depends on stateful handles.**
