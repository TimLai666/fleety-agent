...
| `FLEETY_POLICY` | `auto_review` | Default: sends every non-read tool, including critical operations, to the server's `cheap` model tier and executes only an exact `approve` decision. Explicit `full_access` runs mutate tools directly with audit/rollback, while `require_approval` uses interactive approval. Auto-review never asks a human; reviewer errors, missing context, redaction failures, timeouts, invalid output, and danger signals that the reviewer does not justify all deny. |
...
| `FLEETY_MODEL_EFFORT` | (none) | Default reasoning effort for the main model: `low` / `medium` / `high`. Sent only to models whose family accepts it (OpenAI o-series/gpt-5 → `reasoning_effort`; Gemini 2.5 → thinking budget); otherwise omitted. The agent can raise/lower its own effort via `set_effort` (it applies from the next turn onward, including the next goal-continuation turn), and a parent sets a subagent's effort at spawn. |
| `FLEETY_CHEAP_MODEL_EFFORT` | (none) | Default reasoning effort for the economy tier. |
| `FLEETY_AUTO_EFFORT` | `on` | Auto-select the reasoning effort per top-level user message by difficulty, so a hard task starts in the right gear without waiting for the agent to raise it. Runs one lightweight economy-tier classification per message; a failed/unparseable classification just keeps the default. Only applies when the agent hasn't pinned an effort with `set_effort` (a manual pin wins). `off` disables it — then only a manual pin or the configured default sets effort. The auto pick is not persisted: it is that message's baseline only. |
...
| `start` / `stop` / `restart` | run now / stop now / restart. `start`/`restart` **wait for the process to actually come up** — polling the single-instance pidfile for a live (and, for a restart, *new*) owner — and error if it never does, so a manager "start" that accepted the request but whose binary failed to launch is reported as a failure, not a success. For **fleety-server** a non-forced `restart` **defers until idle** (no in-flight turn) and then waits for that deferred restart to complete (up to ~330 s); `restart --force`, or passing the deferral deadline (~300 s), restarts immediately and interrupts the in-flight turn, which is then recovered from the journal, not lost. For **fleetyd** a manual `restart` is immediate; only its **self-update** path defers until idle (no running on-device tool; deadline ~300 s, cooldown ~30 s). |
