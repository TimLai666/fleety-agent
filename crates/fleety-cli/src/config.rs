...
pub fn run(args: &[String]) -> Result<()> {
    ...
    match parse(args) {
        ...
        ConfigCommand::Edit => edit_screen(&path),
        ConfigCommand::Help => {
            println!(
...
/// `edit` entry point: a ratatui screen on a TTY, else the line-based loop.
fn edit_screen(path: &Path) -> Result<()> {
    if std::io::stdout().is_terminal() {
        run_tui_edit(path)
    } else {
        edit_interactive(path)
    }
}
