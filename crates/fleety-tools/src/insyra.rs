...
mod tests {
    ...
    // Runs only when the built sidecar is provided via FLEETY_INSYRA_BIN; the
    // workspace CI builds it first and sets that. Skips silently otherwise so
    // `cargo test` stays green without the Go toolchain.
    #[tokio::test]
    async fn insyra_exec_roundtrip_when_sidecar_available() {
        let Ok(bin) = std::env::var("FLEETY_INSYRA_BIN") else {
            return;
        };
        if !Path::new(&bin).is_file() {
            return;
        }
        let root = std::env::temp_dir().join(format!("fleety-insyra-rs-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&root).expect("mk temp");
        let mut reg = ToolRegistry::new();
        register_insyra(&mut reg, &root);

        reg.call(
            "insyra_exec",
            json!({ "command": "newdl 1 2 3 4 5 as x", "session": "t" }),
        )
        .await
        .expect("newdl");
        let mean = reg
            .call(
                "insyra_exec",
                json!({ "command": "mean x", "session": "t" }),
            )
            .await
            .expect("mean");
        assert!(mean["output"].as_str().unwrap_or_default().contains('3'));

        // bad command surfaces as an error, not a crash
        assert!(reg
            .call(
                "insyra_exec",
                json!({ "command": "zzz_nope", "session": "t" })
            )
            .await
            .is_err());
